DeployedClassificationModel for MATLAB Production Server

Before you can use the functions packaged by the compiler, you need to do the following:
* Deploy DeployedClassificationModel.ctf to a server instance
* Develop a client application to access the functions.

1. Deploy DeployedClassificationModel.ctf

1.1 Prerequisites to Deployment
Before you can deploy DeployedClassificationModel.ctf, you need a server instance to host 
it. The server instance must be configured to have access to MATLAB Runtime(R2024a).
...
1.2 DeployedClassificationModel.ctf Deployment
To deploy DeployedClassificationModel.ctf, copy it into the server instance's 
`auto_deploy` folder. The server instance will automatically deploy it and make it 
