...
%}
    ...
    function draw(obj, rho, V)

      alf = atan2(V(3), V(1));

      % draw the wing and tail 
      obj.WingProp.draw(rho, V, obj.cm_b)
      obj.Tail.draw(rho, V, obj.cm_b)

      % if there are extra props draw them as well
      for ii = 1:numel(obj.Prop)
        obj.Prop{ii}.draw(obj.cm_b, V)
      end
      ...
      surf(Xc+h_v(1),Yc+h_v(2),Zc+h_v(3),'facecolor','b','edgecolor','none')
