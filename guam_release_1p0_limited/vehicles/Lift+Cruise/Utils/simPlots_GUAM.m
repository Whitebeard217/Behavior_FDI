...
subplot(3,1,2)
%plot(time, [dela Right_Ail.data d_dela]*180/pi)
Left_ail.data = reshape(Left_ail.data, [size(Left_ail.data,3), 1]);
plot(time, [Left_ail.data Right_Ail.data d_dela]*180/pi)
ylabel('aileron','fontsize',15)
legend({'$Left_\delta$ [deg]','$Right_\delta$ [deg]','$\dot{\delta}$ [deg/sec]'}, 'fontsize', 12,'Interpreter','latex')
axis( [min(time) max(time) -35 35])
grid on;
zoom on;
...
ylabel('{\bf Power [kW]}','fontsize',15)
xlabel('time [sec]','fontsize',15)
...
subplot(3,1,3)
plot(time, total_power/1000,'linewidth',1.5)
ylabel('{\bf Power [kW]}','fontsize',10)
xlabel('time [sec]','fontsize',15)
%axis([-1 max(time) 0 1000])
grid on
zoom on
...
ylabel('[ft]','fontsize',15)
xlabel('time [sec]','fontsize',15)
%axis([0 max(time) -50 50])
grid on
zoom on
